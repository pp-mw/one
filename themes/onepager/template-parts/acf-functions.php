...
function get_acf_intro_text( $flexible ) {
	...
	if ( $flexible['intro_text'] ) {
		...
		$content  = '<section id="intro-text" class="row"><div class="section"><h1>' . get_the_title() . '</h1>';
...
function get_acf_location( $flexible ) {
	...
	if ( $flexible['name'] ) {
		...
		$key         = 'AIzaSyApJC60Ov2XrOqVcOWpB3nosGhNANQnND0';
		$address_str = $flexible['name'] . '+' . $flexible['street'] . '+' . $flexible['zipcode'] . '+' . $flexible['location'];
		$q           = str_replace( array( ' ', '&' ), array( '+', '%26' ), $address_str );
		$mapdiv      = $get_tag->tag( 'iframe', null, array( 'src' => 'https://www.google.com/maps/embed/v1/place?key=' . $key . '&q=' . $q ) );

		$return = $get_tag->div( $get_tag->div( $mapdiv, 'map' ) . $get_tag->div( $addressdiv, 'addresses' ), 'locations' );
...
// acf_update_setting( 'google_api_key', 'AIzaSyApJC60Ov2XrOqVcOWpB3nosGhNANQnND0' );
// }
